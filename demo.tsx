		...
		<div className="flex items-center gap-2 mb-2">
			...
			<div className="flex-1">
				...
				<div className="h-4 bg-gray-900 rounded-sm overflow-hidden border border-gray-700">
					<div className={`h-full transition-all duration-300 ${color.replace("text-", "bg-")}`} style={{ width: `${(value / max) * 100}%` }} />
				</div>
			</div>
		</div>
	);
			...
			<div className="relative border-4 border-[#1f2833] rounded-lg shadow-2xl bg-black overflow-hidden">
				...
				{message && (
					<div className="absolute top-10 left-1/2 transform -translate-x-1/2 z-10 animate-bounce">
						<div className="bg-[#45a29e] text-black px-4 py-2 rounded shadow font-bold border-2 border-white">{message}</div>
					</div>
				)}
						...
						<div className="grid grid-cols-2 gap-8 text-left mb-8 text-sm text-gray-400 bg-gray-900 p-6 rounded border border-gray-700">
							<div>
								...
								<ul className="space-y-1">
									<li>
										<span className="bg-gray-700 px-1 rounded">WASD</span> to Move
									</li>
									<li>
										<span className="bg-gray-700 px-1 rounded">SHIFT</span> to Sprint
									</li>
									<li>
										<span className="bg-gray-700 px-1 rounded">E</span> to Interact
									</li>
								</ul>
							</div>
							<div>
				...
				{gameState === "gameover" && (
					<div className="absolute inset-0 bg-red-900/90 flex flex-col items-center justify-center z-20">
						...
						<button onClick={() => window.location.reload()} className="px-6 py-3 bg-white text-red-900 font-bold rounded hover:bg-gray-200">
							TRY AGAIN
						</button>
					</div>
				)}
				...
				<canvas ref={canvasRef} width={MAP_WIDTH * TILE_SIZE} height={MAP_HEIGHT * TILE_SIZE} className="bg-[#1a1a1d] cursor-none" />
			</div>
				...
				{[1, 2, 3, 4, 5, 6, 7, 8].map((slot) => (
					<div
						...
						className={`w-12 h-12 border-2 ${
							slot === 1 ? "border-yellow-400 bg-gray-800" : "border-gray-700 bg-gray-900"
