...
export function DebugPanel() {
  ...
  const [moveSpeed, setMoveSpeed] = useState(160);
    ...
    <div className="fixed top-4 right-4 z-50 w-64 bg-[#1a1410]/95 border-2 border-[#3a2618] rounded-lg p-4 text-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          className="text-[#8b7355] hover:text-white"
        >
          ✕
        </button>
      </div>
      ...
      <div className="space-y-4">
        {/* Camera Zoom */}
        <div>
          <label className="block text-[#8b7355] text-xs mb-1">
        ...
        <div className="pt-2 border-t border-[#3a2618]">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => { setZoom(0.3); setPlayerScale(0.5); }}
              className="px-2 py-1 text-xs bg-[#3a2618] text-[#c9a959] rounded hover:bg-[#4a3628]"
            >
              Zoomed Out
            </button>
            <button
              onClick={() => { setZoom(0.5); setPlayerScale(1.0); }}
              className="px-2 py-1 text-xs bg-[#3a2618] text-[#c9a959] rounded hover:bg-[#4a3628]"
            >
              Default
            </button>
            <button
              onClick={() => { setZoom(1.0); setPlayerScale(1.5); }}
              className="px-2 py-1 text-xs bg-[#3a2618] text-[#c9a959] rounded hover:bg-[#4a3628]"
            >
              Close Up
            </button>
