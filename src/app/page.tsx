...
export default function Home() {
	...
	// Animate between landing1 and landing2
	useEffect(() => {
	...
	return (
		...
		>
          ...
          `,
			...
			{/* Landing artwork - animated between 2 frames */}
			<div className="relative w-full max-w-2xl mb-8">
				<div className="relative aspect-square rounded-lg overflow-hidden border-4 border-[#3a2618] shadow-2xl">
					...
					<Image
						...
						alt="Alin's Basement"
						fill
						className={`object-cover transition-opacity duration-100 ${frame === 1 ? "opacity-100" : "opacity-0"}`}
						priority
					/>
					{/* Frame 2 */}
					<Image
						...
						alt="Alin's Basement"
						fill
						className={`object-cover transition-opacity duration-100 ${frame === 2 ? "opacity-100" : "opacity-0"}`}
						priority
					/>
					{/* Subtle vignette overlay */}
					<div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#1a1410]/60 pointer-events-none" />
				</div>
			</div>
			...
			<div className="w-full max-w-2xl space-y-6 relative z-10">
				...
				<div className="grid gap-3 md:grid-cols-2 mt-8">
					...
					>
						<div className="text-[#c9a959] font-bold uppercase text-xs tracking-wider mb-2">Survival Mechanics</div>
					...
					>
						<div className="text-[#c9a959] font-bold uppercase text-xs tracking-wider mb-2">Your Mission</div>
						<ul className="space-y-1 text-[#8b7355]">
				...
				{/* Tech stack footer */}
				<footer className="text-center text-xs text-[#3a2618] pt-6">Built with Next.js + Phaser + Zustand</footer>
			</div>
		</main>
	);
}
